    ...
    async handleCallback(req, res, next) {
        try {
            ...
            if (error) {
                ...
                return res.redirect(`/login.html?error=${encodeURIComponent(String(error_description || error))}`);
            }
            // Validate parameters
            if (!code || !state) {
                ...
                throw new AppError(ErrorCode.INVALID_REQUEST, 400, { missingCode: !code, missingState: !state });
            }
            // Validate state (CSRF protection)
            if (!stateService.validateState(String(state))) {
                throw new AppError(ErrorCode.INVALID_STATE, 400);
            }
    ...
    async getUser(req, res, next) {
        try {
            ...
            if (oauthService.isTokenExpired(req.session.user.expiresAt)) {
                ...
                try {
                    const refreshedTokenData = await oauthService.refreshAccessToken(req.session.user.refreshToken);
                    ...
                    req.session.user.expiresAt = new Date(Date.now() + refreshedTokenData.expires_in * 1000);
                    req.session.save();
                }
                catch (refreshError) {
