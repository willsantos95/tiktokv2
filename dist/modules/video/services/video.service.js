...
export class VideoService {
    validateVideoFile(file) {
        ...
        if (!VALID_VIDEO_FORMATS.includes(file.mimetype)) {
            ...
            throw new AppError(ErrorCode.INVALID_FILE_FORMAT, 400, { allowedFormats: VALID_VIDEO_FORMATS });
        }
        // Validate file size
        if (file.size > MAX_VIDEO_SIZE) {
            ...
            throw new AppError(ErrorCode.FILE_TOO_LARGE, 413, { maxSize: MAX_VIDEO_SIZE });
        }
    }
    validateMetadata(metadata) {
        if (!metadata) {
            throw new AppError(ErrorCode.INVALID_METADATA, 400);
        }
        if (metadata.title && metadata.title.length > MAX_CAPTION_LENGTH) {
            throw new AppError(ErrorCode.INVALID_METADATA, 400, { message: `Caption exceeds maximum length of ${MAX_CAPTION_LENGTH}` });
        }
        const validPrivacyLevels = ['PUBLIC', 'FRIENDS', 'SELF_ONLY'];
        if (!validPrivacyLevels.includes(metadata.privacyLevel)) {
            throw new AppError(ErrorCode.INVALID_METADATA, 400, { validPrivacyLevels });
        }
    }
    async initializeUpload(user, fileSize) {
        try {
            logger.info('📤 Initializing video upload', {
                fileSize: `${(fileSize / 1024 / 1024).toFixed(2)}MB`,
            });
            const response = await axios.post(`${config.tiktok.apiBaseUrl}/v2/post/publish/video/init/`, {
                source_info: {
                    source: 'FILE_UPLOAD',
                    chunk_size: fileSize,
                },
            ...
            if (axios.isAxiosError(error)) {
                throw new AppError(ErrorCode.TIKTOK_API_ERROR, 500, error.response?.data);
            }
            throw error;
        }
    }
    async uploadVideoChunk(user, uploadToken, videoBuffer, partNumber = 1) {
        try {
            logger.info('📤 Uploading video chunk', {
                size: `${(videoBuffer.length / 1024 / 1024).toFixed(2)}MB`,
                partNumber,
            });
            await axios.post(`${config.tiktok.apiBaseUrl}/v2/post/publish/video/upload/`, videoBuffer, {
                headers: {
                    Authorization: `Bearer ${user.accessToken}`,
            ...
            if (axios.isAxiosError(error)) {
                throw new AppError(ErrorCode.UPLOAD_FAILED, 500, error.response?.data);
            }
            throw error;
        }
    }
    async finalizeUpload(user, uploadToken, metadata, publishType) {
        try {
            ...
            const response = await axios.post(`${config.tiktok.apiBaseUrl}/v2/post/publish/video/finish/`, {
                upload_token: uploadToken,
