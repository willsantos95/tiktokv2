...
export class VideoService {
    ...
    validateMetadata(metadata) {
        if (!metadata) {
            throw new AppError(ErrorCode.INVALID_METADATA, 400, { message: 'Video metadata is required' });
        }
    ...
    async initializeUpload(user, fileSize, chunkSizeBytes = 5 * 1024 * 1024) {
        try {
            ...
            const response = await axios.post(`${config.tiktok.apiBaseUrl}/v2/post/publish/video/init/`, initPayload, {
                headers: {
            ...
            if (axios.isAxiosError(error)) {
                ...
                throw new AppError(ErrorCode.TIKTOK_API_ERROR, error.response?.status || 500, {
    ...
    async uploadVideoChunk(user, uploadToken, videoBuffer, partNumber = 1, chunkSize) {
        try {
            ...
            if (axios.isAxiosError(error)) {
                const errorData = error.response?.data;
                throw new AppError(ErrorCode.UPLOAD_FAILED, error.response?.status || 500, {
    ...
    async finalizeUpload(user, uploadToken, metadata, publishType) {
        try {
            ...
            if (!response.data?.data?.video_id) {
                ...
                throw new AppError(ErrorCode.UPLOAD_FAILED, 500, { message: 'Invalid response from TikTok API - missing video_id' });
            }
            ...
            throw new AppError(ErrorCode.UPLOAD_FAILED, 500, { message: error instanceof Error ? error.message : String(error) });
        }
    }
    async uploadVideoFile(user, uploadToken, filePath, chunkSizeBytes = 5 * 1024 * 1024) {
        try {
            ...
            for await (const chunk of fileStream) {
                ...
                if (currentChunkSize >= chunkSizeBytes || partNumber === chunkCount) {
                    ...
                    await this.uploadVideoChunk(user, uploadToken, buffer, partNumber, fileSize);
                    chunks.length = 0;
                    currentChunkSize = 0;
                    partNumber++;
                }
